...
if __name__ == "__main__":
  """Outputs a !javascript code! for graph.
    ...
    This is a very terrible hack, and should be used only for fast local
    development. Problem is, that we are creating explicit javascript code.
    Reason for this is, that for security reasons you can not read a local json
    file and you would need a server for actually passing the data to
    visualization.
    """
  if len(sys.argv) <= 2:
    print("input, output")
  ...
  graph = create_graph(event_data, verbose=True)
